---
    title: Organisation and Tenant Isolation Standards
    ...
    # Chapter 7 - Organisation and Tenant Isolation Standards

    ## 1. Purpose

    This chapter defines the mandatory engineering standard for organisation and tenant isolation standards in the F1 platform.

    The purpose of this standard is to make Organisation, Tenant, Isolation implementation predictable, reviewable and subordinate to the Product Specification. It establishes how engineers and AI coding agents SHALL apply the volume concern without inventing product behaviour, routes, states, events, schema objects, operational commitments or commercial values.

    ## 2. Scope

    This chapter governs implementation decisions, design review, verification evidence and repository changes related to organisation and tenant isolation standards.
...
- governance/QUALITY_STANDARD.md
    ...
    For organisation and tenant isolation standards, reviewers SHALL pay particular attention to Organisation, Tenant, Isolation. Any design that makes this concern the owner of business truth SHALL be rejected unless a higher-authority document explicitly grants that ownership.
    ...
    Observability for organisation and tenant isolation standards SHALL support review of whether the correct owner executed the work, whether authority was checked, whether idempotency or concurrency protections applied, and whether any deferred decision blocked execution.
    ...
    Compliance with this chapter is mandatory for all repository changes touching organisation and tenant isolation standards. Non-compliance SHALL be corrected before merge or explicitly waived through the accepted governance process with traceability to the approving authority.
