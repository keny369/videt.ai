---
title: Authority Hierarchy and Canonical Ownership
identifier: EM-XII-002
version: 1.0
status: Normative
owner: Engineering Governance
---

# Chapter 2 - Authority Hierarchy and Canonical Ownership

## 1. Purpose

This chapter defines the mandatory engineering standard for authority hierarchy and canonical ownership in the F1 platform.

The purpose of this standard is to make Authority, Hierarchy, Canonical, Ownership implementation predictable, reviewable and subordinate to the Product Specification. It establishes how engineers and AI coding agents SHALL apply the volume concern without inventing product behaviour, routes, states, events, schema objects, operational commitments or commercial values.

## 2. Scope

This chapter governs implementation decisions, design review, verification evidence and repository changes related to authority hierarchy and canonical ownership.
...
Authority SHALL be resolved by scope before rank, as required by PM-REQ-003 and defined normatively in Engineering Manual Volume I, EM-I-003. This chapter restates that model for repository stewardship and SHALL NOT diverge from it.

Every decision SHALL first be classified into exactly one authority scope. Only that scope's precedence ladder SHALL then be applied. An artefact holds no authority outside its own scope. Outside its scope an artefact is inapplicable rather than outranked.

On product behaviour, the canonical owner is the Product Specification, and precedence SHALL be: constitution and governance; foundation layer 000 through 020; the ADR registry comprising accepted ADRs and ratified Owner Decisions integrated into their canonical owner; volume specifications; derived implementation artefacts. The Engineering Manual holds no product-behaviour authority at any rank.

On engineering practice, the canonical owner is the Engineering Manual, and precedence SHALL be: constitution and governance; accepted ADRs; the Engineering Manual; source code and tests; operational and informative material.

Where the scope of a decision is itself disputed, the affected implementation path SHALL stop until the canonical owner resolves the scope. Scope SHALL NOT be selected to reach a preferred outcome.
...
For authority hierarchy and canonical ownership, reviewers SHALL pay particular attention to Authority, Hierarchy, Canonical, Ownership. Any design that makes this concern the owner of business truth SHALL be rejected unless a higher-authority document explicitly grants that ownership.
...
Observability for authority hierarchy and canonical ownership SHALL support review of whether the correct owner executed the work, whether authority was checked, whether idempotency or concurrency protections applied, and whether any deferred decision blocked execution.
...
Compliance with this chapter is mandatory for all repository changes touching authority hierarchy and canonical ownership. Non-compliance SHALL be corrected before merge or explicitly waived through the accepted governance process with traceability to the approving authority.
