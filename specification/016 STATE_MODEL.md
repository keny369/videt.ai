...
| Session | active, revoked, expired | active only when WF-001 atomically consumes a valid purpose-bound Identity Validation Receipt after the exact Account/Organization/assurance checks | revoked on Account/Organization suspension, revocation, closure, or explicit security revocation; expired when server time reaches idle or absolute expiry | active to revoked; active to expired | revoked or expired to any state; extension of absolute expiry; refresh of the same Session | Identity and Access Context; session lifecycle service performs timed expiry | exact sign-in replay returns the same Session; distinct fresh receipts may create concurrent Sessions without a count limit and do not revoke one another; accepted activity advances idle expiry only; at equality expiry wins; no automatic sign-in retry or Session refresh exists | revoked and expired are terminal | reauthentication with a fresh receipt creates a distinct Session; no terminal Session reopens | SessionCreated, SessionRevoked, SessionExpired |
...
| Document | discovered, ingested, parsed, indexed, quarantined, retired | discovered after an accepted crawl/import fetch creates the Document and its ingestion lineage | retired after retention or revocation removes the Document from further product use | discovered to ingested; ingested to parsed; parsed to indexed; discovered, ingested, parsed, or indexed to quarantined; indexed or quarantined to retired | discovered to parsed or indexed; ingested to indexed; same-state transitions; retired to any state | Intake and Retrieval Contexts; Data Lifecycle Context for quarantine and retirement | ingestion, parsing, and indexing retries occur on their Job records and never move or regress a Document on failure; each failed Job retains the exact workflow reason while the Document remains at its last committed state | Job failure is not a Document state; quarantine is a separately authorized lifecycle transition; terminal state retired | a successful authorized replacement Job advances from the last committed state; corrected bytes create a new immutable Document version; authorized quarantine disposition may retire the Document; retired never reopens | DocumentDiscovered, DocumentIngested, DocumentParsed, DocumentIndexed, DocumentQuarantined, DocumentRetired |
