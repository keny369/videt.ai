...
| `POST /api/v1/organizations/:organization_id/projects/:project_id/reassessments` | `StartReassessment` | `reassessment.trigger` | no | unavailable under `UPSTREAM-V1-REASSESSMENT-TRIGGER-EVENT-010`; 202 only after correction |
...
| `POST /api/v1/organizations/:organization_id/projects/:project_id/reassessment-schedules` | `ActivateReassessmentSchedule` | `reassessment.trigger` | policy conditional | unavailable under `UPSTREAM-V1-REASSESSMENT-TRIGGER-EVENT-010`; 201 only after correction |
...
This transport applies only to nonmetered reads. The five Volume I low-cost operations cannot be enabled until `UPSTREAM-V1-LOW-COST-METERING-005` defines the durable-response unit and idempotency identity for composite HTML/Turbo/JSON reads. Their proposed physical paths are retained below as blocked mappings, not routable endpoints.
...
| `GET /api/v1/organizations/:organization_id/projects/:project_id/issues` | `QRY-009 IssueCollection` | `issue.read` | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/issues/:id` | `QRY-010 IssueDetail` | `issue.read`; Evidence fields independently redacted | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/issues/:id/evidence` | `QRY-036 IssueEvidenceCollection` | `issue.read`; each Evidence item is independently full or reference-only under Evidence field/classification authorization | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/issues/:id/closure-evidence` | `QRY-037 IssueClosureEvidenceCollection` | `issue.read`; each Evidence item is independently full or reference-only under Evidence field/classification authorization | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/issues/:id/adjudication-cases` | `QRY-038 IssueAdjudicationCaseCollection` | `issue.read`; Case fields follow the Issue's current field-authorization result | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/scores/current` | `QRY-024 CurrentScore` | `score.summary.read` or `score.detail.read` by field | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/history` | `QRY-025 HistoryCollection` | `history.read` | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/history/compare` | `QRY-008 HistoryComparison` | `history.read` | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` and `UPSTREAM-V1-COMPARISON-EVENT-007` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/recommendations` | `QRY-011 RecommendationCollection` | `recommendation.read` | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/recommendations/:id` | `QRY-012 RecommendationDetail` | `recommendation.read` | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/recommendations/:id/rationale-evidence` | `QRY-039 RecommendationRationaleEvidenceCollection` | `recommendation.read`; each Evidence item is independently full or reference-only under Evidence field/classification authorization | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
| `GET /api/v1/organizations/:organization_id/projects/:project_id/action-queue` | `QRY-013 ActionQueue` | `recommendation.read` | blocked by `UPSTREAM-V1-LOW-COST-METERING-005` |
...
| `DocumentQuarantined`, `DocumentRetired` | unavailable | unavailable | unavailable | unavailable under `UPSTREAM-V1-DOCUMENT-LIFECYCLE-012` | unavailable |
...
| `IssueFingerprintCollision` | unavailable | unavailable | unavailable | unavailable under `UPSTREAM-V1-ISSUE-COLLISION-013` | unavailable |
...
| `ReassessmentTriggered` | WF-011 | unavailable | unavailable | unavailable under `UPSTREAM-V1-REASSESSMENT-TRIGGER-EVENT-010` | unavailable |
...
| `ComparisonGenerated` | WF-012 | unavailable | unavailable | unavailable under `UPSTREAM-V1-COMPARISON-EVENT-007` | unavailable |
...
| `RoleExpiryBlocked` | WF-013 | unavailable | unavailable | unavailable under `UPSTREAM-V1-ROLE-EXPIRY-BLOCKED-EVENT-011` | unavailable |
...
| `SessionRevoked`, `SessionExpired` | WF-001/WF-013 | `session` | ST | `none` | transition; standalone explicit revocation unavailable under `UPSTREAM-V1-SESSION-REVOCATION-002` |
...
`UPSTREAM-V1-SESSION-REVOCATION-002`: The Session state model permits an `active -> revoked` transition by "explicit security revocation", but Volume I defines no current-user sign-out/revocation command, actor, permission, trigger, error result, audit reason, or acceptance oracle. Account/Organization lifecycle revocation remains implemented through its defined workflows; Volume II exposes no standalone Session revocation endpoint until Volume I decides whether such a product action exists.

`UPSTREAM-V1-READ-AUTHORIZATION-004`: the absent read routes named above remain absent; a mutation permission or role label is never reused as an inferred read permission.

`UPSTREAM-V1-LOW-COST-METERING-005`: the five low-cost operations require one Entitlement Decision and exactly one LowCostUsageRecord for an authorized response, but Volume I does not define whether a composite page plus Turbo frames is one response or several, how a safe read transports replay identity, or how `304` is counted. The marked query routes and their HTML/Turbo counterparts remain disabled.

`UPSTREAM-V1-REACTIVATION-PROOF-006`: WF-013 requires Account reactivation to have valid identity but does not say whether this is the administrator's current Session, fresh proof for the suspended target Account, or another artifact, and defines no purpose, freshness, subject binding or outward precedence. The `ReactivateAccount` Application operation therefore has no physical route or attributes schema.

`UPSTREAM-V1-COMPARISON-EVENT-007`: WF-012 calls history comparison a read with no domain transition but names `ComparisonGenerated`. Volume II cannot decide whether every read emits it, projection construction emits it, or the event is absent. Comparison remains disabled even after the metering issue until this is corrected.

`UPSTREAM-V1-ORGANIZATION-REACTIVATION-PROOF-008`: Organization suspension revokes every Session, Organization reactivation requires fresh managed identity plus MFA, ordinary sign-in rejects the inactive Organization, and no reactivation receipt purpose, inactive-tenant entry path or proof transport exists. Volume II exposes no Organization-reactivation route or control.
...
`UPSTREAM-V1-REASSESSMENT-TRIGGER-EVENT-010`: WF-011 requires `ReassessmentTriggered` with exact manual/scheduled provenance but does not say whether an admitted precreation Entitlement Block emits it. That branch has a terminal Reassessment Result and no Evaluation; the execution branch has an Evaluation. Choosing Reassessment Result, Evaluation, Entitlement Decision or Scheduled Action as the affected entity, or omitting/emitting the event in one branch, changes the logical event envelope and observable notification/audit stream. Volume II therefore exposes neither manual `StartReassessment` nor `ActivateReassessmentSchedule`, creates no substitute route and serializes no `ReassessmentTriggered` until controlled Volume I correction fixes occurrence and affected identity. Existing retained history remains readable subject to the separate read blockers.

`UPSTREAM-V1-ROLE-EXPIRY-BLOCKED-EVENT-011`: WF-013 requires `RoleExpiryBlocked` when timer expiry would remove the last effective OrganizationAdmin, but defines no immutable decision identity/type/value/status/authority/policy tuple, no lifecycle transition, no attempt/failure profile, and no retry timing from which the required event profile fields can be derived. The Role Assignment contract also mandates a SecurityOperator/OrganizationAdmin notification, while the accepted WF-014 trigger table defines no `RoleExpiryBlocked` route predicate, selectors, Required Permission, severity or `notification_context_v1` variant. Volume II serializes no `RoleExpiryBlocked`, invents no decision record and guesses no notification route/context. Ordinary Role Assignment expiry continues through the named lifecycle service when it is not blocked; it has no public expiry route. The last-administrator invariant still denies every unsafe authority removal.

`UPSTREAM-V1-DOCUMENT-LIFECYCLE-012`: the foundation state model permits Document transitions from discovered/ingested/parsed/indexed to `quarantined` and from indexed/quarantined to `retired` and names `DocumentQuarantined`/`DocumentRetired`, but WF-005/WF-006 define only discovered-to-ingested-to-parsed-to-indexed. Volume I supplies no producer, trigger, actor/service authority, permission or command, reason precedence, idempotency/replay contract, error result or acceptance oracle for quarantine/retirement, and the Application Layer has no operation. Volume II exposes no route or internal substitute and serializes neither event until controlled Volume I correction. Defined ingestion/parsing/indexing transitions remain available.

`UPSTREAM-V1-ISSUE-COLLISION-013`: the Issue replay/collision contract says a second different-preimage tuple with the same fingerprint *may* create its own Issue. Creating versus omitting that Issue changes sealed Issue-set membership, score inputs and customer-visible history, while Volume I fixes neither branch, Evaluation continuation/failure nor an affected identity for `IssueFingerprintCollision`. Volume II therefore admits no second-Issue collision branch and serializes no `IssueFingerprintCollision` until controlled Volume I correction chooses mandatory creation or mandatory omission and closes those consequences. Check Result collision remains independently implementable: its candidate identity is preallocated in the Evaluation slot and its exact collision mapping is defined above.
