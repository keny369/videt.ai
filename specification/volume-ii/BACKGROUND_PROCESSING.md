...
4. Set each row to `claimed`, increment `claim_generation`, set a claim lease **derived per :288** (the scheduler REQUESTS 30 seconds; the written lease is `max(60 seconds, product_attempt_deadline - claim_time + 30 seconds)` capped absolutely at 15 minutes), create/exact-replay the restricted Work Dispatch Binding for the registry-selected target, and commit.
...
The producer evaluates that predicate in its command transaction and inserts the route row with the event and captured Notification Policy identity. The consumer never discovers a route from naming convention, free text, recipient guess, current policy, or role label. All other admitted event types have zero consumer routes. Pipeline advancement, search/index work, deterministic projection rebuild and timers use an explicitly persisted ScheduledAction or `work_executions` identity created by their owning operation; they are not inferred from a domain event. The baseline frontend has no asynchronous UI-invalidation transport. Under OD-013 Option 1 there are no platform or cross-Organization scope classes to emit: every event is Organization-scoped, and a platform-wide Incident or cross-Organization Investigation is represented as coordinated per-Organization records each emitting their own Organization-scoped events. The pre-Organization bootstrap events `BootstrapGrantIssued` and `BootstrapGrantExpired` carry the immutable bootstrap principal in `organization_id` under the DM-REQ-013 gate WF-001 expressly permits. Physical event and route exposure for these remains intentionally deferred to the Volume II baseline.
...
Lease duration is `max(60 seconds, product_attempt_deadline - claim_time + 30 seconds)` capped at 15 minutes. The cap is absolute: no caller-supplied lease request may exceed it. A handler whose work can legitimately exceed 15 minutes divides it into checkpointed members; no individual claim exceeds the cap. Heartbeat interval is one third of the lease duration, rounded down to whole seconds and bounded from 5 through 30 seconds.

> **CORRECTED 2026-07-30 BY DECISIONS ADR-095, from a 30-second floor to 60.** The 30-second floor was internally inconsistent with this section's own cadence rule and with F-01's ratified maximum hop, and the inconsistency was reachable rather than theoretical. A lease must satisfy `lease > interval + max_hop`, where `interval` is this section's one-third cadence and `max_hop` is the resolver timeout (`Ceilings::DNS_TIMEOUT_MAX_S`, 15 seconds, taken OUTSIDE the per-hop deadline) plus the ratified hard request timeout (15 seconds). At a 30-second lease that reads `30 > 10 + 30`, which is false, so a single ratified redirect hop could outlive a lease held at the floor — the state a derived lease takes near a run deadline and for every action kind that stamps no `product_attempt_deadline` at all. At 60 it reads `60 > 20 + 30`. The sentence "the cap is absolute" was likewise added rather than assumed: the first implementation expressed the cap as `greatest(15 minutes, caller_request)`, which is not a cap above 15 minutes, and the omission was only harmless because both callers happened to request less.
...
Reassessment slot insertion and dispatch are canonical under OD-025, which removes the trigger event rather than binding it: no `ReassessmentTriggered` occurrence or affected record is required because no trigger event exists. Provenance is retained on the Reassessment Result record and its Audit Evidence, and a run is observable through `ReassessmentScheduleEvaluated`, `EvaluationStarted`, `ReassessmentCompleted`, `ReassessmentFailed` and `ReassessmentCanceled`; no replacement event is introduced. Background exposure remains intentionally deferred until the Volume II baseline. A `role_assignment_expire` action may terminalize an ordinary accepted expiry, but when the last-OrganizationAdmin invariant blocks expiry the action MUST NOT terminalize the expiry. Under OD-026 the branch is canonical: it writes one immutable `RoleExpiryBlockDecision` and emits exactly one `RoleExpiryBlocked` from the role-expiry lifecycle service, in the same transaction as its authorization and denial Audit Evidence, and routes the mandatory security notification. The Assignment remains `active` and effective past `expires_at_utc` until the guard clears, and the branch is re-evaluated on each Organization authorization-epoch advance rather than replayed from a transport queue. Background exposure of this behaviour remains intentionally deferred until the Volume II baseline and is not defined by this correction package.

No Document-quarantine or Document-retirement action kind, work type, lifecycle branch or inferred parser consequence exists because OD-015 removes the `quarantined` and `retired` Document states and their events; no such action kind or lifecycle branch exists. Parsing/indexing failure leaves the Document at its last accepted state exactly as Volume I specifies.
...
Under OD-013 Option 1 `OpenInvestigation` atomically freezes the ordered required-input plan and creates one `investigation_collection_checkpoints` row plus one due-now `investigation_input_collect` action for every required input, for exactly one Organization-owned Investigation record. An investigation spanning several Organizations creates one such Investigation per affected Organization, linked by a shared `correlation_id`. Background exposure remains intentionally deferred to the Volume II baseline. Historically, execution was unavailable because Volume II cannot persist the required cross-Organization events differently.
