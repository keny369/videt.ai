...
OD-024 removes `ComparisonGenerated` under ADR-019: the WF-012 comparison read emits no domain event, is side-effect-free, and discharges its audit obligations entirely through Audit Evidence with correlation ID. The previously undefined outcome set, repeated-view trigger and idempotency identity no longer arise, because no event exists. The semantic contract is canonical in Volume I; the corresponding application-layer and transport exposure remains intentionally deferred until the Volume II baseline and is not defined by this correction package.
...
OD-025 removes `ReassessmentTriggered` under ADR-019 through a PM-REQ-009 controlled foundation change to the WF-011 coverage row in `018 OBSERVABILITY.md`. The question of whether an admitted trigger that Entitlement then blocks emits the event no longer arises, because no trigger event exists; provenance is retained on the Reassessment Result record and its Audit Evidence, and execution is observable through `ReassessmentScheduleEvaluated`, `EvaluationStarted`, `ReassessmentCompleted`, `ReassessmentFailed` and `ReassessmentCanceled`. Historically: That branch creates a failed Reassessment Result with no Evaluation, while an executable run creates and starts an Evaluation. Emission for every admitted trigger, only after Evaluation creation, and different affected entity/profile choices produce different event histories. Schedule activation, due-slot execution and manual reassessment start remain unreachable until Volume I fixes the event occurrence, affected record, profile and replay identity.

### `UPSTREAM-V1-ROLE-EXPIRY-BLOCKED-EVENT-011`

WF-013 names `RoleExpiryBlocked` when expiry would remove the last effective Organization Administrator, and OD-026 supplies the immutable `RoleExpiryBlockDecision` `decision` record carrying the mandatory event-profile fields, together with epoch-driven re-evaluation in place of a retry or terminal schedule. The Assignment remains active and effective past `expires_at_utc` until the guard clears, and no lifecycle status is added. The Role contract also requires a mandatory security/OrganizationAdmin notification, while the accepted WF-014 trigger table defines no route predicate, selectors, permission, severity or context for this event. The last-admin block branch is canonical under OD-026 and its exposure is deferred to the Volume II baseline; ordinary nonblocking Role Assignment expiry remains defined.

### `UPSTREAM-V1-DOCUMENT-LIFECYCLE-012`

OD-015 removes the `quarantined` and `retired` Document states and the `DocumentQuarantined` and `DocumentRetired` events under ADR-019 through a PM-REQ-009 controlled foundation change to `016 STATE_MODEL.md`. The Document lifecycle is `discovered -> ingested -> parsed -> indexed` and `indexed` is terminal; the states are removed rather than reserved, so no migration shape, operation, job, event or DML grant may admit either. Evidence quarantine is a separate, unaffected state machine. Historically: WF-005/WF-006 define only discovered-to-ingested-to-parsed-to-indexed execution and supply no quarantine/retirement trigger, actor or service authority, permission, command, reason precedence, idempotency, recovery or acceptance behavior. Automatic Evidence-driven quarantine, retention-driven retirement, an administrator command and no executable path are observably different. Volume II recognizes the reserved states in migration shape but exposes no operation, job, event or DML grant for either transition.

### `UPSTREAM-V1-ISSUE-COLLISION-013`

OD-017 resolves the Issue fingerprint contract under ADR-019: on a same-hash/different-preimage collision the second Issue MUST NOT be created and the affected Evaluation fails closed using the existing canonical collision outcome and telemetry. The prior divergence between the fingerprint contract's permissive “may create” and AC-SM-006 no longer admits different Issue-set membership, Evaluation completion or downstream score, Recommendation and history results. Restricted collision telemetry is retained; no duplicate Issue variant is persisted.
