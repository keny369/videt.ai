...
Organization-home data, Project/Source/Crawl/Evaluation collections, the Notification inbox and Export enumeration each have an explicit read row under ratified OD-020 — `organization.read`, `project.read`, `source.read`, `crawl.read`, `evaluation.read`, `notification.inbox.read` and `export.list` — and render only on that exact tenant-scoped grant, which is never implied by holding a companion mutation permission. Integration status, Support-Session collection, Incident/Investigation/Legal-Hold views, deletion-job collections, privileged Billing surfaces and broad administration collections remain deny-by-default under OD-020's ratified carve-out, pending OD-034, and MUST NOT render at all, and in particular MUST NOT render using a mutation permission, approval authority, support-session scope, recipient binding, requester status, ownership or Role-name inference. The home logical destination renders `organization.read` data and, for each product-data region, only that region's own exact named read contract. Exact Evidence, entitlement-notice, command-result and Export-retrieval scopes remain available; the score, Issue, history and Recommendation reads are separately unreachable under OD-019's fail-closed leg pending OD-035, which is a metering question and not a read-authority one.

Every screen or Frame mapped or potentially mapped to `report.view`, `history.view`, `issue.read`, `recommendation.read` or `score.read` MUST NOT be made reachable by selecting a convenient operation or by counting a full page and its Frames ad hoc. They remain unreachable under OD-019's ratified fail-closed leg pending OD-035: no route declares one of the five operations, so each resolves `operation_unknown` and returns Block with `contact_support`. This includes apparently single-purpose History, Issue, Recommendation and Current Score responses. Historical comparison carries no additional withholding: under ratified OD-024 `ComparisonGenerated` is removed, the WF-012 comparison read is audit-only and side-effect-free, and OD-024 records comparison reads unblocked. Its only withholding is the metering one above.
