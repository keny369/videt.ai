...
The handler itself remains read-only even when Volume I requires request-attributable evidence. No query may supply a product-event intent unless Volume I names that event. `QRY-008` is not executable while `UPSTREAM-V1-COMPARISON-EVENT-007` leaves the exact `ComparisonGenerated` trigger/result/idempotency behavior unresolved.
...
| `QRY-002 ProjectCollection` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; no `project.read` action exists | Project read store | `ProjectCollectionDTO` |
...
| `QRY-004 SourceCollection` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; management actions are not inferred as read authority | Source/scope/verification read store | `SourceCollectionDTO` |
| `QRY-005 SourceDetail` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; management actions are not inferred as read authority | Source, policy and immutable run references | `SourceDetailDTO` |
| `QRY-006 EvaluationCollection` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004` and `UPSTREAM-V1-LOW-COST-METERING-005` | Evaluation/readiness/coverage read store | `EvaluationCollectionDTO` |
...
| `QRY-014 NotificationCollection` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; recipient status and route permission are not inbox-read authority | Notification/recipient/Delivery projection | `NotificationCollectionDTO` |
| `QRY-015 ExportCollection` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; `export.retrieve` authorizes one known retrieval, not enumeration | Export state and manifest metadata | `ExportCollectionDTO` |
| `QRY-016 ExportDetail` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; exact command result/retrieval remains available to its requester | Export, approval and manifest read store | `ExportDetailDTO` |
| `QRY-017 AccessAdministration` | blocked for broad collection disclosure by `UPSTREAM-V1-READ-AUTHORIZATION-004`; exact command-result reads remain scoped to their actor | Identity/access projections | `AccessAdministrationDTO` |
...
| `QRY-019 LifecycleAdministration` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; exact command/audit results remain available only in their expressly defined scope | closure, hold, deletion and tombstone projections | `LifecycleAdministrationDTO` |
| `QRY-020 SecurityOperations` | blocked as a combined collection by `UPSTREAM-V1-READ-AUTHORIZATION-004`; mutation/support authority is not inferred as collection read authority | restricted security projections | `SecurityOperationsDTO` |
...
| `QRY-022 CrawlCollection` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; `crawl.trigger` is not inferred as read authority | Crawl/coverage projection | `CrawlCollectionDTO` |
| `QRY-023 CrawlDetail` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004` | Crawl, Source/URL outcome and stage summary projection | `CrawlDetailDTO` |
...
| `QRY-028 IntegrationStatus` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; Integration mutation/support authority is not read authority | Integration/Credential metadata projection with no secret reference | `IntegrationStatusDTO` |
| `QRY-029 SupportSessionCollection` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; exact command results remain available to their submitting/deciding actors | Support Session projection | `SupportSessionCollectionDTO` |
| `QRY-030 IncidentCollection` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; `incident.respond` is not inferred as collection read authority | Incident projection | `IncidentCollectionDTO` |
| `QRY-031 IncidentDetail` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; exact command results remain actor-scoped | Incident/playbook/attempt/restoration projection | `IncidentDetailDTO` |
| `QRY-032 InvestigationDetail` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; Support Sessions authorize named investigation input reads, not an undefined general detail response | Investigation/custody/gap/report projection | `InvestigationDetailDTO` |
| `QRY-033 LegalHoldCollection` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; `legal_hold.manage` is not inferred as collection read authority | LegalHold projection; no held payload | `LegalHoldCollectionDTO` |
| `QRY-034 DeletionJobCollection` | blocked by `UPSTREAM-V1-READ-AUTHORIZATION-004`; an own-Organization `deletion.retry` command result does not authorize a collection | deletion-job/manifest-outcome projection | `DeletionJobCollectionDTO` |
...
The Volume I Permission Baseline defines score, Issue, history, Recommendation, Evidence, entitlement-notice and exact Export-retrieval reads but no standalone Organization-home data, Project, Source, Crawl, general Evaluation, Notification-inbox, Account/policy administration, Export enumeration/detail, Integration status, Support-Session collection, Billing summary, Incident/Investigation/Legal-Hold collection/detail or deletion-job collection read action. Inferring read authority from a mutation permission, Role name, Assignment scope, recipient status, requester status or ownership would produce different disclosure behavior. The `organization_home` sign-in destination authorizes only a shell, not an unnamed data disclosure. Queries with an exact existing read contract remain implementable once any separate metering blocker is corrected; the affected registry rows stay unreachable except for their separately authorized command-result or named workflow-input scope. Volume II MUST NOT invent `project.read`, `source.read`, `evaluation.read`, `notification.read`, `export.read` or an equivalent permission.
...
WF-012 names `ComparisonGenerated` for a workflow triggered when a user opens the historical comparison view, but it does not define whether the event occurs for compatible, insufficient, not-comparable and unavailable outcomes; whether reload, Turbo prefetch/frame fetch or repeated selection emits again; or the event's idempotency identity. Treating a safe GET as one event per HTTP request, one per selected pair, or no event until a successful compatible result produces different audit/outbox behavior. `QRY-008` and its physical routes remain disabled; Volume II MUST NOT emit, suppress or deduplicate `ComparisonGenerated` until controlled Volume I clarification supplies the exact trigger, outcome set and replay identity.

### `UPSTREAM-V1-ORGANIZATION-REACTIVATION-PROOF-008`

WF-013 requires Organization reactivation through fresh managed identity plus MFA after suspension has revoked every human Session. The only accepted Identity Validation Receipt purposes are the four WF-001 purposes; none admits Organization reactivation, while `existing_account_sign_in` rejects an inactive Organization. Volume II cannot invent a fifth logical receipt purpose, reuse a sign-in receipt contrary to its purpose, accept raw provider proof, or let a revoked Session authenticate the command. `ReactivateOrganization` remains registered but unreachable until controlled Volume I clarification supplies the proof artifact, subject/Organization binding, freshness/consumption, failure precedence and physical entry behavior.
...
WF-011 names `ReassessmentTriggered` with exact manual or scheduled provenance but does not say whether an admitted trigger that is subsequently blocked by Entitlement emits it. The allowed Entitlement-block branch creates a failed Reassessment Result whose current Evaluation is null, while the successful branch creates and starts an Evaluation. Therefore a fixed affected entity/profile cannot represent both branches, and emitting only after Evaluation creation versus emitting for every admitted trigger produces different observable event history. `ActivateReassessmentSchedule`, `EvaluateReassessmentSlot` and `StartReassessment` remain registered but unreachable; no route, due-slot action or event mapping may choose the trigger point or affected entity until controlled Volume I clarification defines the exact occurrence set, affected record, profile and replay identity.

### `UPSTREAM-V1-ROLE-EXPIRY-BLOCKED-EVENT-011`

WF-013 requires `RoleExpiryBlocked` when timed expiry would violate the last-OrganizationAdmin invariant, but defines no immutable decision/attempt/failure record supplying the mandatory logical event profile fields, and no subsequent retry or terminal schedule behavior. The Role contract additionally mandates a security/OrganizationAdmin notification, but WF-014 supplies no trigger route, selector, permission, severity or notification-context contract for it. A decision event, failed timed attempt and retained retryable schedule action would carry different identities, values and lifecycle consequences; recipient choices would also differ. Ordinary nonblocking Role Assignment expiry remains defined; the last-admin block branch of `ExpireRoleAssignment` is unreachable and MUST NOT emit a guessed event, route a guessed notification or schedule a guessed retry until controlled Volume I clarification supplies all three contracts.

### `UPSTREAM-V1-DOCUMENT-LIFECYCLE-012`

The foundation state model admits Document quarantine and retirement and names their events, but WF-005/WF-006 contain no trigger, actor/service authority, permission, command, reason precedence, idempotency, recovery or acceptance contract for either transition. No `QuarantineDocument`, `RetireDocument`, retention timer, Evidence-derived mutation or equivalent operation is registered. Volume II MUST NOT infer one from Data Lifecycle ownership or parsing failure; the reserved persistence states remain unreachable until controlled Volume I clarification supplies the behavior.

### `UPSTREAM-V1-ISSUE-COLLISION-013`

The Issue fingerprint body permits a same-hash/different-preimage tuple to create its own Issue, while AC-SM-006 requires an altered tuple to create a distinct Issue. Those instructions permit different Issue-set membership and downstream results. `ExecuteChecks` may still complete the accepted Check Result path, and Check Result key collision remains governed by its existing nonmerge/fail-closed contract. When Issue derivation encounters this exact Issue-fingerprint collision predicate, however, no handler may choose whether to persist the second Issue, emit `IssueFingerprintCollision`, call `SealIssueSet`, fail or continue the Evaluation, or publish any dependent score, Recommendation or history state. That branch remains unreachable until controlled Volume I correction makes creation mandatory or prohibited and fixes Evaluation continuation/failure.
