...
| --- | --- | --- | --- |
| WF-001 Onboarding And Sign-In | Self-service emits two ordered `BillingStateChanged` events ending active, `OrganizationActivated`, and `ProjectCreated` with `project_state=draft` under one correlation; invitation acceptance emits `InvitationAccepted`; existing-account sign-in emits `SessionCreated` with its creation reason and destination decision. `SourceActivated` and `ProjectActivated` are separate later outcomes. | Audited rejected command outcome with the exact branch/sign-in reason and no partial write or Session; transaction timeout/exhaustion reports the exact onboarding or sign-in reason. No failure-only Organization, Account, BillingEntity, or Project state is created. | Chief Product |
...
| WF-011 Reassessment Schedule And Execution | Every ordinary or latest-coalesced due slot emits one `ReassessmentScheduleEvaluated`; an admitted run that reaches Evaluation creation emits one `EvaluationStarted` inside that atomic commit; successful replacement emits `ReassessmentCompleted` plus atomic projection events. Every admitted run is provenance-complete without a dedicated trigger event: `trigger_kind`, nullable policy identity, version and content hash, slot number and due time are retained on the Reassessment Result record and its Audit Evidence under SM-REQ-004, and a manual run's provenance is carried by its command Audit Evidence. An admitted run that Entitlement blocks creates no Evaluation and is observable through `ReassessmentFailed`, its linked Entitlement Decision, and Audit Evidence. | `superseded_policy`, `inactive_scope`, `active_evaluation_conflict`, and `ineligible` are measured separately from `ReassessmentFailed`; coalescing is missed-slot range/count metadata, never an outcome. Lateness, duplicate-slot suppression, due-time and evaluation-time Organization/Project state versions, restoration boundary, entitlement Block, and failed/canceled run retain exact policy/slot/correlation identities. | Chief Architect |
...
| WF-014 Notification Delivery | In-app delivered and Mailgun accepted/delivered are known success; local deduplication hits prove no repeated submission of one claimed attempt; `NotificationDeliveryReconciled` identifies an exact provider-evidence resolution. | `NotificationDeliveryFailed` is known terminal failure; `NotificationDeliveryAcceptanceUnknown`/`NotificationUncertain` are operational uncertainty, never counted as confirmed success or failure, and require one escalation within 5 minutes. Metrics include submissions, definitive retries, uncertainty count/age, reconciliation results, and acknowledged replays. | Chief Rails |

OBS-REQ-022: Coverage table workflows MUST remain synchronized with state and error models.

An attempt-failure signal is not a terminal-failure signal when the corresponding bounded retry remains available. Provider-acceptance uncertainty is neither attempt failure nor terminal failure and MUST have its own status, metric, alert, and age distribution. `ExportRevoked` is an authorized terminal lifecycle outcome, not a generation or delivery failure, and MUST NOT be counted in the Export failure rate.
