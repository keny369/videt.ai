...
| WF-011 Reassessment Schedule And Execution | Every ordinary or latest-coalesced due slot emits one `ReassessmentScheduleEvaluated`; an admitted scheduled or manual run emits one provenance-complete `ReassessmentTriggered`, and successful replacement emits `ReassessmentCompleted` plus atomic projection events. | `superseded_policy`, `inactive_scope`, `active_evaluation_conflict`, and `ineligible` are measured separately from `ReassessmentFailed`; coalescing is missed-slot range/count metadata, never an outcome. Lateness, duplicate-slot suppression, due-time and evaluation-time Organization/Project state versions, restoration boundary, entitlement Block, and failed/canceled run retain exact policy/slot/correlation identities. | Chief Architect |
