...
6a. **Total request deadline** — RATIFIED EVOLUTION, owner decision 2026-08-06, `DECISIONS.md` ADR-141, opened as FU-43. One `fetch` has ONE wall-clock boundary. DNS resolution, connection setup, TLS negotiation, response headers, body reads **and every redirect hop** spend the same budget; **no hop re-arms any timeout**; when the budget is exhausted the client stops **deterministically** and reports the ratified `timeout` outcome. The per-attempt connect+response deadline of property 6 is retained only as a **subordinate ceiling**: the effective timeout for every operation is the **lesser** of the per-attempt ceiling and the remaining total budget. The total is itself clamped to the same hard platform ceiling as the per-attempt deadline, so one `fetch` can never exceed it however many hops it follows. **A caller cannot bypass the total by supplying only a per-attempt timeout**: the total defaults to the per-attempt value, which is the tightest form, and no shape of the call is unbounded.

   *Why this is an evolution and not a clarification.* Property 6 was implemented as a per-attempt deadline **re-armed at every redirect hop**, which the S-07-009 acceptance review measured at **11.1x** the caller's bound at the ratified 10-redirect ceiling, and up to 22x including DNS. No caller-side fix existed, because the façade accepted only a per-attempt number and the sole lever was `max_redirects`. `deadline_at` was therefore not a real boundary: a run that was over could still be reading a customer's site. Executable negative evidence — a maximum-length redirect chain that must not continue past the deadline, measured on the wall clock, with a control that must still complete — is `spec/platform/outbound/total_deadline_spec.rb`.
