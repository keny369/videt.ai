...
- Status: Accepted register; thirty-five owner decisions comprising one resolved objectively, twenty-six ratified or resolved by owner decision on 2026-07-17 and integrated under ADR-019 and ADR-020, one withdrawn, and seven pending with deterministic interim behaviour. OD-001 through OD-011 predate the RC1 review; OD-012 through OD-032 were registered by the RC1 correction programme under ADR-018, and OD-034 and OD-035 by the Volume II freeze-candidate programme under ADR-023, in each case because the derivation proved that existing authority does not force a unique answer for them. The seven pending decisions are OD-014, OD-023, OD-027, OD-031, OD-032, OD-034 and OD-035. Each records deterministic fail-closed interim behaviour and an exact blocking impact, and none blocks the Volume I freeze or the Volume II baseline.
- Status Authority: Each decision's own `Current Status` field is the sole status authority for that decision. This Status line is a summary and is not a status source; where it and a decision's `Current Status` disagree, the field prevails and this line is defective. That rule is executed by `scripts/validate_volume_ii.py`, which resolves status from `Current Status` alone.
...
- Classification: C
- Current Status: Resolved on 2026-07-17 by explicit Product Owner decision recorded in [OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md](OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md); integrated by ADR-020
- Approved Option: Option 1 — `retention-interim-v1` is approved as the fixed Volume I retention baseline. Product scope is worldwide availability; principal initial English-speaking markets are the United States, United Kingdom, Australia, New Zealand, Canada and South Africa. Qualified external legal counsel reviewed the product and retention position and raised no objection. No customer-configurable retention is approved; configurability remains disabled and is never inferred from a fixed policy.
...
- Ratified Behavior: The exact `retention-interim-v1` policy in [../015 DATA_LIFECYCLE.md](../015%20DATA_LIFECYCLE.md) is the approved fixed Volume I retention baseline, not an interim. Its per-class minimum and maximum windows, legal-hold authority, backup deletion deadlines and destruction-evidence rules are approved as recorded. No customer may change any window. Legal hold suspends irreversible destruction only, never access revocation or product-validity expiry. Account and Organization deletion use the exact LifecycleDeletionJob, the 30-day primary and 35-day post-primary backup deadline, restore tombstones, and immutable Deletion Evidence. A conflicting legal obligation fails closed and blocks destruction. Approval is evidenced under the amended Qualified Legal Approval standard below: an append-only owner approval record, a factual record that qualified external counsel reviewed the position and raised no objection, the identified retention baseline, and the applicable worldwide product and principal-market scope. Privileged legal material is excluded from this repository by design and its absence is not a gap.
- Required Owner And Legal Approval Package: The approved policy package MUST identify one immutable version and digest and record each decision below separately; approval of one item does not imply approval of another:
  ...
  - Qualified legal approval (amended 2026-07-17 under ADR-020): this standard originally demanded repository-hosted counsel signatures, reviewer identities and package digests. Privileged legal material was never intended to live in the engineering repository, so the standard is narrowly amended to accept, and to be satisfied by, exactly the following four elements: (1) an append-only owner approval record; (2) a factual record that qualified external legal counsel reviewed the position and raised no objection; (3) the identified retention baseline; and (4) the applicable product and market scope. Counsel identities, privileged communications, workpapers, internal legal records, package attestations and signature artifacts are expressly excluded from this repository, and their absence is by design rather than a gap. The repository MUST NOT fabricate a signature, reviewer identity, package digest or legal opinion, and MUST NOT present any record as a legal opinion. Nothing else in OD-011's approval package is relaxed: the retention, hold, destruction, backup, audit and customer-configuration decisions each remain required and are recorded above.
- Latest Responsible Decision Point (met on 2026-07-17): Before storing production customer data or making a contractual retention claim.
- Blocking Impact: None for Volume I. OD-011 is closed: the retention baseline is approved for worldwide availability with the recorded principal markets, and storing production customer data is no longer blocked by this decision. Customer-configurable retention remains out of scope rather than blocked pending approval, and requires controlled change plus a new owner decision.
- ADR Threshold: Met; recorded in ADR-020. Not met for customer-configurable retention, a new legal authority role, or a materially different backup/destruction architecture, none of which is approved.
- Owner Required: Chief Security and Chief Product following qualified legal review
- Approval Record: Closed by explicit Product Owner decision on 2026-07-17, recorded in [OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md](OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md) and integrated by ADR-020.
...
- Classification: C
- Current Status: Resolved on 2026-07-17 by explicit Product Owner decision recorded in [OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md](OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md); integrated by ADR-020
- Approved Option: Option 1 for every sub-decision — pre-Organization bootstrap uses the DM-REQ-013 substitution that WF-001 now expressly names, and a platform-wide Incident or cross-Organization Investigation is represented as coordinated per-Organization records, each canonically owned by exactly one Organization. No `event_scope` discriminator, no platform-owned or cross-Organization-owned canonical event entity, and no foundation change.
...
- Ratified Behavior: Every workflow event and audit record carries a nonnull `organization_id` resolving to an existing Organization. The sole exception is the pre-Organization bootstrap substitution that DM-REQ-013 already reserves for the named onboarding contract, which WF-001 now expressly names, making `BootstrapGrantIssued` and `BootstrapGrantExpired` emittable under that existing foundation gate without any foundation change. Coordination across Organizations is orchestration over Organization-owned records and never creates a platform-owned canonical record: a platform-wide Incident is represented as one Organization-scoped Incident record per affected Organization, and a cross-Organization Investigation as one Organization-scoped Investigation record per affected Organization, each emitting one Organization-scoped event per affected Organization and linked by `correlation_id`, extending the per-Organization pattern WF-018 already establishes for Export. Canonical ownership is always singular and always an Organization; coordination is a relationship between owned records, not an owner. No producer may emit with null, a sentinel, an invented platform tenant, an arbitrarily selected Organization, or an omitted tenant field. The business ability to coordinate platform-wide incidents and cross-Organization investigations is preserved in full.
- Latest Responsible Decision Point (met on 2026-07-17): Before Volume II acceptance, and ahead of the other pending decisions rather than at the deadline, because every further Volume II event, audit, or tenant-scope section written against the unresolved envelope widens the correction surface.
- Blocking Impact: None. OD-013 is closed; the logical event and audit envelope, the `domain_events` and `audit_records` DDL, and the WF-017/WF-018 scope columns are unblocked, and `UPSTREAM-V1-EVENT-SCOPE-001` is retired.
- ADR Threshold: Met; recorded in ADR-020. Option 1 alters neither the event envelope nor the tenancy model and requires no PM-REQ-009 foundation change; it consumes the DM-REQ-013 gate that already exists.
...
- Classification: C
- Current Status: Resolved on 2026-07-17 by explicit Product Owner decision recorded in [OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md](OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md); integrated by ADR-020
- Approved Option: Option 2 — WF-007 produces `EvidenceRetentionExpiring`, its Trigger extends to include the Evidence Payload retention deadline, and the integrity-validation service is the authority.
...
- Ratified Behavior: WF-007 owns production of `EvidenceRetentionExpiring`. Its Trigger includes the `product_evidence_payload` retention deadline, and the event is emitted alongside `EvidenceValidationChanged` with the integrity-validation service as the named service authority. Exactly one warning is emitted per Evidence Payload at 30 days before its current `product_evidence_payload` retention maximum, carrying the Evidence identity, its Organization, the capture cursor, the maximum instant, and the correlation ID, and it requests reassessment through the accepted WF-011 path. No new workflow, capability, decision record, Evidence Payload state, or notification route variant is created. This satisfies the 30-day expiry-warning criterion of AC-CAP-013, which had no producer before this decision.
- Latest Responsible Decision Point (met on 2026-07-17): Before the first production Evidence Payload reaches 23 months, that is, before any real payload crosses the 30-day threshold.
- Blocking Impact: None. OD-029 is closed; `EvidenceRetentionExpiring` has exactly one producer and AC-CAP-013's 30-day warning criterion is satisfiable.
- ADR Threshold: Met; recorded in ADR-020.
- Owner Required: Chief Architect and Chief Product
- Approval Record: Closed by explicit Product Owner decision on 2026-07-17, recorded in [OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md](OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md) and integrated by ADR-020.
...
- Classification: C
- Current Status: Resolved on 2026-07-17 by explicit Product Owner decision recorded in [OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md](OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md); integrated by ADR-020
- Approved Option: Option 3 — path-split at the maximum instant: the LifecycleDeletionJob and its Deletion Evidence remain the Account-deletion and Organization-closure mechanism only, retention-expiry destruction is proved by a distinct `security_audit` deletion audit record, and destruction stays on the existing capture cursor and 24-month maximum rather than accelerating to the accrued minimum.
...
- Ratified Behavior: Destruction of an Evidence Payload whose effective validation status is already `invalid` proceeds on the exact `product_evidence_payload` schedule in [../015 DATA_LIFECYCLE.md](../015%20DATA_LIFECYCLE.md): cursor at capture, 24-month elapsed maximum. Destruction occurs at that maximum instant and is not accelerated to the accrued 30-day minimum. The LifecycleDeletionJob and its immutable Deletion Evidence remain the Account-deletion and Organization-closure mechanism only and are not extended to retention expiry. Retention-expiry destruction is proved instead by a distinct immutable `security_audit` deletion audit record carrying the Evidence identity, its Organization, the capture cursor, the maximum instant, the destruction instant, and the correlation ID. No Evidence Validation Decision is appended, which is why a separate audit artifact rather than a validation record proves the destruction. A legal hold suspends irreversible destruction and the payload is destroyed at the first eligible instant after release. No retention window is changed by this decision.
- Latest Responsible Decision Point (met on 2026-07-17): Before the first production Evidence Payload destruction on either the retention-expiry or Account-deletion and Organization-closure path, and before any contractual claim about destruction evidence.
- Blocking Impact: None. OD-030 is closed; already-invalid Evidence Payload destruction has an exact procedure, instant, and immutable audit artifact.
- ADR Threshold: Met; recorded in ADR-020.
- Owner Required: Chief Architect and Chief Security
- Approval Record: Closed by explicit Product Owner decision on 2026-07-17, recorded in [OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md](OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md) and integrated by ADR-020.
...
- Classification: B
- Current Status: Resolved on 2026-07-17 by explicit Product Owner decision recorded in [OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md](OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md); integrated by ADR-020
- Approved Option: Option 3 — the `queued to completed` edge is removed from [../016 STATE_MODEL.md](../016%20STATE_MODEL.md) by PM-REQ-009 controlled change, every LifecycleDeletionJob enters `running`, and WF-013's original enumeration is restored.
...
- Ratified Behavior: No LifecycleDeletionJob may transition `queued` directly to `completed`; that edge is removed from the canonical state model rather than conditioned, and any attempt is an invalid transition rejected and logged under SM-REQ-007. The lifecycle is asynchronous: every job enters `running` before it may complete, and successful completion occurs only from `running` with immutable Deletion Evidence, including where the frozen resource and class manifest is empty or every member is already provably destroyed at admission. The owner treats the direct edge as unintended. No compatibility path preserves it, and no schema, migration, function or grant may admit it.
- Latest Responsible Decision Point (met on 2026-07-17): Before the Volume II architecture baseline is frozen, because the `lifecycle_deletion_jobs` state CHECK constraint binds it.
- Blocking Impact: None. OD-033 is closed; the foundation edge is removed and WF-013's enumeration and the state model agree.
- ADR Threshold: Met; recorded in ADR-020, which records the PM-REQ-009 controlled foundation change removing the edge.
- Owner Required: Chief Architect
- Approval Record: Closed by explicit Product Owner decision on 2026-07-17, recorded in [OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md](OWNER_DECISION_RECORD_2026-07-17_LEGAL_AND_CLOSURE.md) and integrated by ADR-020.

### OD-034 Read Authority For Security, Administrative And Internal Operational Objects

- Decision ID: OD-034
- Successor To: OD-020
- Successor Basis: OD-020's Ratified Behavior and Blocking Impact each delegate this scope expressly: security, administrative and internal operational objects "remain deny-by-default pending a separate decision", which is "out of scope for this decision rather than resolved by it".
- Exact Question: By what mechanism, and to which roles under which conditions, does an actor acquire read authority over the security, administrative and internal operational object classes that OD-020 expressly left deny-by-default — Support Session, Incident, Investigation, Legal Hold, Emergency Access Grant, LifecycleDeletionJob and other deletion jobs, and privileged Billing surfaces — and are Integration/Credential status, Account administration and Policy administration reads inside or outside that scope?
- Classification: C
- Current Status: Pending owner approval
- Why The Decision Exists: OD-020 is ratified and resolved read authority for customer-facing objects by adding explicit rows to `permission-baseline-v1`. It expressly did not resolve this scope. Its Ratified Behavior states that security, administrative and internal operational objects "remain deny-by-default pending a separate decision", and its Blocking Impact states that scope "is out of scope for this decision rather than resolved by it". [WORKFLOW_SPECIFICATIONS.md](WORKFLOW_SPECIFICATIONS.md) carries the same reservation as normative binding text and names one further class — Emergency Access Grant — that OD-020's own sentence omits. That separate decision was never registered: it had no identifier, no owner, no interim statement and no entry in this register, so no artifact could withhold against it or implement it. This registration supplies the missing node and changes no behaviour. The `Permission Baseline` is a closed enumeration and [WORKFLOW_SPECIFICATIONS.md](WORKFLOW_SPECIFICATIONS.md) denies an undefined permission by default, so these reads are already denied; what is missing is the authority to cite for that denial and the eventual grant. Three sub-questions are genuinely open and no accepted authority selects among them: whether the reads land as rows in `permission-baseline-v1` or in a separately versioned read profile; which of the seven canonical roles receives which read under which condition; and whether the three classes OD-020's problem statement raised but neither carve-out sentence named — Integration/Credential, Account administration and Policy administration — are inside this scope or are separately customer-facing.
- Affected Capabilities: CAP-021, CAP-023, CAP-024, CAP-025
- Affected Workflows: WF-013, WF-014, WF-015, WF-017, WF-018
- Affected Product Rules: PRULE-034, PRULE-037, PRULE-038, PRULE-042, PRULE-044
- Affected Foundation Requirements: SEC-REQ-004, SEC-REQ-008, SEC-REQ-013, SEC-REQ-027
- Affected Acceptance Criteria: AC-PRULE-044
- Acceptance Criteria Considered And Expressly Not Gated: AC-CAP-021, AC-CAP-023, AC-CAP-025, AC-WF-013, AC-WF-017, AC-WF-018 and AC-PRULE-034 each name one of these object classes, and each was examined against this decision. None is gated by it. Their assertions are command, approval, collection and notification behaviour that is fully determined under deny-by-default: every dual-control step is satisfied by its own approval contract, which carries the canonical command request hash and the expected target state version as asserted command inputs rather than by a general read; every "exact command result" remains available to its acting actor, which no Volume II slice disputes; and PRULE-034's uncertain aggregate, escalation and user status is served by the ratified `notification.inbox.read` row. Only AC-PRULE-044 is owner-gated, because its `complete permission-matrix allow/deny fixtures` clause cannot be satisfied for an object class that has no row in the table at all. That is a Volume I coverage boundary, not an implementation defect.
- Options:
  1. Add explicit per-object read rows to `permission-baseline-v1` for each named class, each with a full role/condition matrix, exactly as OD-020 Option 1 did for customer-facing objects. Reuses the three scope-condition vocabularies the table already defines — `authorized incident/adjudication scope only`, `authorized investigation scope only`, and `support-session only` — and introduces no new authorization mechanism.
  2. Introduce a separate versioned `read_profile_version` artifact alongside the existing `permission_profile_version` / `classification_profile_version` / `protected_permission_profile_version` triple, enumerating object class to minimum role set, so security and administrative reads version independently of tenant reads.
  3. Keep deny-by-default permanently for every named class and expose no read surface for them, treating each object as reachable only through its own command results.
- Recommended Option: Option 1 for the classes OD-020 named, subject to owner selection of every cell, because it extends the accepted table shape and reuses conditions the table already carries. Option 2 only if support, incident, investigation, legal-hold and deletion reads must version separately from tenant reads — which is OD-020's own conditional recommendation handed forward to this decision and is the central open question. Option 3 is not recommended because it makes AC-PRULE-044's fixture-completeness clause permanently unsatisfiable rather than answering it. The specific role/condition cells remain OWNER INPUT REQUIRED: Volume I names no read action for any of these classes, `CAPABILITY_MODEL.md:16` states an `Actor` line never grants authority, a Support Session scopes an already-permitted action and is never itself a read grant, and OD-020 expressly rejected deriving read from write. Selecting a cell by inference would therefore invent authority no accepted document wrote.
- Evidence Supporting The Recommendation:
  - SEC-REQ-008 requires roles to map to explicit permission sets with deny-by-default behavior, so the read must land in the table rather than in tenant policy; an Access Policy cannot turn a baseline deny into an unconditional allow.
  - The table already carries `security.notice.read` as "allow in incident scope" and `entitlement.notice.read` as an allow, so Volume I already distinguishes a governed *notice* read from an *object* read. That distinction is the nearest accepted precedent and bounds any grant.
  - OD-020 Option 2 (derive read from write) was considered and rejected there for weakening SEC-REQ-013 least privilege; that rejection is inherited and is not reopened here.
  - A re-baselined `permission-baseline-v1` that is not byte-equivalent renders every existing Organization's bootstrap policy `access_policy_invalid`, so the profile-version cost is real and is the reason Option 2 exists.
- Benefits: Completes the read half of the authorization model, makes PRULE-044's complete-fixture requirement satisfiable for every object class, and gives operators and administrators a defined, auditable read surface.
- Costs: Option 1 requires owner decision on every role/condition cell for each class and a new profile version with a re-baselined byte-equivalent `access-policy-v1` payload; Option 2 adds a fourth profile axis to resolve, version and test; Option 3 has no build cost and a permanent operability cost.
- Risks: An implied or over-broad grant widens exposure of the most sensitive object classes in the product — support sessions, incidents, investigations, legal holds and billing. The interim denies and exposes nothing, so the risk is entirely on the grant side.
- Security Implications: This decision sets the read half of the authorization model for the security surface itself. Any new read class carrying classified fields must resolve against the `classification_profile_version=score-visibility-v1` ceilings; Volume I does not currently extend those ceilings to these classes, so the decision must state the ceiling rather than assume one.
- Data Implications: Determines whether a new profile axis exists and what every affected read model may return. Investigation, incident and billing classes carry classified and privileged fields.
- AI Implications: None.
- Operational Implications: Under the interim no operator or administrator can read Support Session, Incident, Investigation, Legal Hold, deletion-job or privileged Billing state through a defined permission. Operational visibility for those classes depends on this approval. Every command, approval and audit path remains fully available.
- Commercial Implications: None direct. The affected classes are internal and security surfaces, not packaged customer features.
- Reversibility: High while the interim holds because nothing is disclosed. Once read authority is granted, data already disclosed cannot be un-disclosed.
- Safe Interim Behavior: Every named class remains deny-by-default exactly as OD-020 ratified and as [WORKFLOW_SPECIFICATIONS.md](WORKFLOW_SPECIFICATIONS.md) binds. No read action is added to `permission-baseline-v1` for Support Session, Incident, Investigation, Legal Hold, Emergency Access Grant, LifecycleDeletionJob or any other deletion job, privileged Billing surface, Integration/Credential status, Account administration or Policy administration. No mutation, approval, support, ownership, recipient, requester or Role-name authority is inferred as read authority, and no wildcard read exists. The exact result of a command remains available to its acting actor in its expressly defined scope, which is existing accepted behaviour and is not widened here. No collection or detail read surface for a named class is routable. This interim approves no OD-034 option and is fully implementable: it is the behaviour every completed Volume II slice already contracts.
- Latest Responsible Decision Point: Before any Volume II read surface for a named class — logical API index or show action, UI list or detail screen, or export — is designed, and before operational readiness for support, incident and investigation work is assessed.
- Blocking Impact: Volume II — no. Implementation — no. Production — no. Feature — yes for any security, administrative or internal operational read surface, which remains absent under the interim. The interim is deterministic, fail-closed and already contracted by every affected slice, so the baseline is implementable without this decision. AC-PRULE-044's fixture-completeness clause is satisfiable for every enumerated cell and remains unsatisfiable for an object class with no row until this decision lands; that limb is withheld and is recorded on MTX-095.
- ADR Threshold: Not met for Option 1 or Option 3, which extend or preserve the accepted table shape and introduce no new authorization mechanism. Required for Option 2, which adds a profile axis to the accepted authorization model.
- Owner Required: Chief Product and Chief Security
- Exact Approval Wording: “I approve OD-034 Option [selected] and, for each of Support Session, Incident, Investigation, Legal Hold, Emergency Access Grant, LifecycleDeletionJob and other deletion jobs, privileged Billing surfaces, Integration/Credential status, Account administration and Policy administration, I approve the read action, the per-role allow/deny/conditional cells, the classification ceiling and the scope condition recorded in the accompanying package, and I confirm whether Integration/Credential, Account administration and Policy administration are inside this decision's scope or are customer-facing under OD-020.”

### OD-035 Low-Cost Read Route-To-Operation Declaration And `report.view` Read Surface

- Decision ID: OD-035
- Successor To: OD-019
- Successor Basis: OD-019's Ratified Behavior requires a static route-to-operation declaration and its Costs record the exhaustive declaration table and `report.view`'s read surface as unpaid; neither is supplied, and OD-019's own Why The Decision Exists classifies the residual assignment as a commercial choice.
- Exact Question: Which one of the five low-cost operations does each metered read route statically declare, and what is `report.view`'s read surface — or is `report.view` retired from the low-cost operation set?
- Classification: C
- Current Status: Pending owner approval
- Why The Decision Exists: OD-019 is ratified and resolved the metered-read *unit*: root-request subsumption, one LowCostUsageRecord per accepted top-level document read, with Frames carrying the root Decision ID. It did not resolve the *assignment*. Its Ratified Behavior requires that "Every metered read route carries a static declaration of exactly one of the five low-cost operations; a read route with no declaration resolves `operation_unknown` and returns Block with `contact_support`". Its Costs record the consequence as unpaid: "Requires a new normative exhaustive route-to-operation declaration table, and `report.view` must be given a defined read surface." Neither has been supplied. WF-015 names `report.view`, `history.view`, `issue.read`, `recommendation.read` and `score.read` and gives each a limits row; beyond that membership Volume I defines none of them — `report.view`, `history.view` and `score.read` appear nowhere else, no document maps any read surface, screen or route to an operation, and `report.view` has no surface at all. The five operation strings are not permissions: three do not exist in `permission-baseline-v1`, and the overlap of `issue.read` and `recommendation.read` between the two vocabularies is a coincidence, so the declaration cannot be inferred from a route's permission cell. OD-019's own Why The Decision Exists names the residual ambiguity directly — "the same navigation can reasonably resolve to `report.view` or `score.read`" — and classifies the consequence as "a commercial and packaging choice rather than an architectural inference", because the assignment determines what the shared 800/1,000 `baseline_reads` bound costs a customer per navigation. Volume II must not make a commercial choice, and [FRONTEND_ARCHITECTURE.md](../volume-ii/FRONTEND_ARCHITECTURE.md) already prohibits making a screen reachable "by selecting a convenient operation".
- Affected Capabilities: CAP-024
- Affected Workflows: WF-015
- Affected Product Rules: PRULE-040
- Affected Foundation Requirements: QA-REQ-009, QA-REQ-010, SEC-REQ-006, OBS-REQ-004
- Affected Acceptance Criteria: AC-CAP-024, AC-WF-015, AC-PRULE-040
- Acceptance Criteria Considered And Expressly Not Gated: AC-PRULE-039 governs the high-cost reserve path only and is unaffected. AC-PRULE-046 governs envelope completeness for the low-cost usage commits that occur and is satisfiable for each of them. AC-CAP-018 and AC-CAP-019 assert field visibility, redaction consistency and comparison outcomes, each fully determined at the query handler and DTO independently of transport metering.
- Options:
  1. Root-page assignment: each metered top-level document read declares the operation naming its principal content — the dashboard and any composite report page declaring `report.view`, and single-purpose History, Issue, Recommendation and Current Score reads declaring `history.view`, `issue.read`, `recommendation.read` and `score.read` respectively. `report.view`'s surface is defined as the composite project overview.
  2. Retire `report.view` from the low-cost set by controlled Volume I change and declare every metered route against one of the remaining four, so the composite project overview declares `score.read`. Removes the undefined surface rather than defining it.
  3. Define `report.view` as the exhaustive surface for every composite page, so any read returning more than one content class declares `report.view` and only genuinely single-class reads declare their named operation.
- Recommended Option: Option 1 as the narrowest reading consistent with root-request subsumption, but the choice is genuinely open and no accepted authority prefers any option. The assignment for `/scores/current` is OWNER INPUT REQUIRED and is the case OD-019 itself names as reasonably resolving either way; its permission cells are `score.summary.read` and `score.detail.read`, which match neither candidate operation. The treatment of the three Evidence child collections is also OWNER INPUT REQUIRED: OD-019 ratified that "Evidence and entitlement-notice reads are not among the five operations, so they are unmetered standalone and subsumed when nested" while also ratifying that "a Frame reached by direct navigation is itself a root", so a directly navigated Evidence collection is both an unmetered Evidence read and a metered root under the same ratified text.
- Evidence Supporting The Recommendation:
  - WF-015 establishes root subsumption for high-cost nesting so that "nesting cannot double meter"; Option 1 is the low-cost analogue and is consistent with CAP-024's never-double-count Success Condition.
  - Option 2 removes a ratified operation and therefore requires a controlled Volume I change to WF-015's enumeration and limits table.
  - Option 3 makes the dashboard's cost stable but leaves `report.view` claiming most navigations, which changes what the shared bound means from what the OD-006 interim assumed.
- Benefits: Makes every metered read route reachable, gives the shared `baseline_reads` bound a stable and explicable meaning per navigation, and closes OD-019's last unpaid cost.
- Costs: Requires a normative exhaustive route-to-operation declaration table, an operation-declaration column on the Volume II route table, and a defined surface for `report.view` or its removal.
- Risks: A wrong or absent declaration changes charging silently; the fail-closed leg is what prevents that today. Inferring a declaration from a route's permission cell would bind billing to a vocabulary Volume I keeps deliberately separate.
- Security Implications: None new. The undeclared-route leg already Blocks server-side through `operation_unknown` with `contact_support` and emits the standard denial audit event; metering resolution is never a client-side or implementation choice.
- Data Implications: Determines the operation recorded on every Entitlement Decision and LowCostUsageRecord, and therefore what every customer's recorded usage means.
- AI Implications: None. `ai.generate` remains a separate high-cost operation.
- Operational Implications: Route-to-operation declarations become a maintained normative artifact, and page composition changes cost if declarations are not maintained.
- Commercial Implications: Direct and primary. The assignment determines what the shared 800/1,000 `baseline_reads` bound buys, and therefore packaging, overage warning timing and margin control.
- Reversibility: Medium. The unit can change through a versioned policy, but any change restates the meaning of every customer's recorded usage.
- Safe Interim Behavior: OD-019's ratified fail-closed leg applies exactly as written and is the interim. No metered read route carries an operation declaration, so every one of them resolves `operation_unknown` and returns Block with `contact_support`, emitting the standard denial audit event. No Entitlement Decision allows and no LowCostUsageRecord is appended on a metered read path. The affected routes are registered as blocked mappings rather than routable endpoints, which is the state Volume II already contracts. No implementation may select a convenient operation, infer one from a permission cell, count a page and its Frames ad hoc, or make a metered route reachable by any other means. Reads that are not among the five low-cost operations — the current Session, the Organization home shell, entitlement notices, security notices, the pending verification challenge, Evidence and entitlement-notice reads standalone, and the customer-facing collections OD-020 ratified — are unaffected and remain fully reachable. This interim approves no OD-035 option.
- Latest Responsible Decision Point: Before commercial packaging is finalized, and before any metered read surface is enabled in a customer environment.
- Blocking Impact: Volume II — no. Implementation — no. Production — no. Feature — yes for the five metered low-cost read responses, which remain unreachable under the interim. The interim is deterministic, fail-closed and fully specified: the route, the query handler, the DTO, the entitlement checkpoint and the Block path are each contracted, so implementation proceeds and the declaration is simply absent. The withheld limb is recorded on MTX-024, MTX-040 and MTX-091.
- ADR Threshold: Not met for Option 1 or Option 3, which supply a declaration table within the accepted metering contract. Required for Option 2, which removes a ratified low-cost operation by controlled Volume I change to WF-015.
- Owner Required: Chief Product
- Exact Approval Wording: “I approve OD-035 Option [selected], and I approve the exhaustive route-to-operation declaration table recorded in the accompanying package, including the operation declared by the composite project overview, by `/scores/current`, and by each directly navigated Evidence child collection, together with the defined read surface for `report.view` or its removal from the WF-015 low-cost operation set.”
