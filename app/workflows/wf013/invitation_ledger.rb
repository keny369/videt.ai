...
module Workflows
  module Wf013
    # The ledger plumbing shared by the WF-013 administrator commands that create
    # and approve Invitations: the durable command execution, the PRULE-044
    # authorization decision recorded on allow AND deny, the audit record, the
    # event envelope, the command result and the idempotency record.
    #
    # Only generic plumbing lives here. Every workflow-specific decision — which
    # capability, which state transition, which event type and profile, which
    # first-match order — stays visible in each handler, exactly as
    # `Workflows::Wf001::InvitationTerminalSupport` does for the receipt-bound
    # terminal commands.
    #
    # Including handlers MUST define `ACTION`, `TARGET_TYPE`,
    # `SUPPORTED_SCHEMA_MAJOR` and `canonical_payload_json`.
    module InvitationLedger
      ...
      # One event, canonically encoded, with the actor attribution and Organization
      # epoch the envelope requires and a null service identity.
      def write_event(store, ids, org, ctx, command, actor, now, aggregate_version, invitation_id,
                      request_sha256, key_digest, requester, type, profile, extra = {})
        envelope = {
          "account_id" => actor.account_id, "actor_id" => actor.account_id,
          "affected_entity_id" => invitation_id, "affected_entity_type" => self.class::TARGET_TYPE,
          ...
          "service_identity_id" => nil, "workflow_id" => "WF-013"
        }.merge(extra)
        ...
        store.insert_event(
          ...
          aggregate_id: invitation_id, aggregate_version: aggregate_version, partition_month: month(now),
      ...
      # An audited no-state outcome: execution, authorization decision, audit and
      # result, no domain write and no event.
      def deny(command:, ctx:, pg:, store:, auth_store:, actor:, decision:, org:, now:, offer: nil,
               invitation_id:, outward:, internal:)
        ids = %i[execution audit result decision].to_h { |k| [k, ctx.generate_id] }
        key_digest = Digest::SHA256.digest(command.idempotency_key)
        request_sha256 = denial_request_hash(command, ctx, actor, offer)

        write_execution(store, command, ctx, org, ids[:execution], invitation_id, actor, request_sha256,
                        key_digest, now, self.class::ACTION)
        write_authorization_decision(auth_store, ids[:decision], ctx, command, actor, decision, now,
                                     invitation_id, self.class::ACTION)
        write_audit(store, ids[:audit], org, ctx, command, invitation_id || command.command_id, actor,
                    to_state: nil, outcome: "failure", reason_code: internal,
                    payload: { "outcome" => "failure", "internal_reason" => internal,
                               "outward_reason" => outward, "invitation_id" => invitation_id,
      ...
      def denial_request_hash(command, ctx, actor, offer)
        Platform::CanonicalJson.digest({
          ...
          "command_payload" => { "offer" => offer ? offer[:uniqueness].unpack1("H*") : nil }
        })
      end
