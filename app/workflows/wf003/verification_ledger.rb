...
module Workflows
  module Wf003
    # The ledger plumbing for the WF-003 Verification commands: the durable command
    # execution, the authorization decision recorded on allow AND deny, the audit
    # record, the domain-event envelope, the command result and the idempotency
    # record. It is the exact WF-002/WF-004 shape — the same
    # `IdentityAccess::Infrastructure::ActorLedgerWriters` platform ledgers and the
    # same canonical envelope — specialized to `WF-003` and the Verification Request
    # aggregate.
    #
    # Including handlers MUST define `ACTION`, `TARGET_TYPE` and `WORKFLOW_ID`.
    module VerificationLedger
      ...
      # One canonically-encoded WF-003 domain-event envelope. The payload carries the
      # Request and Source identity, the method, canonical host and expiry, and NEVER
      # the plaintext challenge token or any raw DNS/HTTP content (contracts/S-05.json
      # event_payload).
      def write_event(store, ids, org, ctx, command, actor, now, aggregate_version, verification_request_id,
                      request_sha256, key_digest, type, profile, extra = {})
        envelope = {
          "account_id" => actor.account_id, "actor_id" => actor.account_id,
          "affected_entity_id" => verification_request_id, "affected_entity_type" => self.class::TARGET_TYPE,
          ...
          "outcome" => "success", "project_id" => command.project_id, "reason_code" => nil,
          "requester_account_id" => actor.account_id, "schema_version" => "1.0",
          "service_identity_id" => nil, "source_id" => command.source_id,
          "verification_request_id" => verification_request_id, "state_version" => aggregate_version,
          "workflow_id" => self.class::WORKFLOW_ID
        }.merge(extra)
        ...
        store.insert_event(
          ...
          aggregate_id: verification_request_id, aggregate_version: aggregate_version, partition_month: month(now),
      ...
      def write_result_success(store, ids, command, ctx, org, actor, now, payload, verification_request_id)
        store.insert_command_result(
          ...
          target_refs: JSON.generate({ self.class::TARGET_TYPE => verification_request_id }),
      ...
      # An audited no-Request outcome for an authenticated actor: execution,
      # authorization decision, audit and result, no Verification Request and no
      # event. `decision` is the real decision — an allow for a tenant/state/duplicate
      # refusal of an authorized actor, a deny for a capability refusal.
      def deny(command:, ctx:, store:, auth_store:, actor:, decision:, org:, now:, request_sha256:,
        ...
        write_audit(store, ids[:audit], org, ctx, command, command.command_id, actor,
                    ...
                    payload: { "outcome" => "failure", "internal_reason" => internal,
                               "outward_reason" => outward, "organization_id" => org,
                               "project_id" => command.project_id, "source_id" => command.source_id }, now:)
      ...
      def schema_failure(command, ctx) = in_memory_failure(command, ctx, "verification_request_schema_unsupported")

      # A pre-transaction failure with no authenticated actor and no proved context
      # to write under: the unsupported-method rejection and the authentication
      # reasons (invalid Session, inactive Account, inactive Organization).
