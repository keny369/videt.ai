...
module Workflows
  module Wf002
    # The ledger plumbing for the WF-002 Project commands: the durable command
    # execution, the authorization decision recorded on allow AND deny, the audit
    # record, the ProjectCreated event envelope, the command result and the
    # idempotency record. It is the exact WF-013 shape — the same
    # `IdentityAccess::Infrastructure::ActorLedgerWriters` platform ledgers and the
    # same canonical envelope — specialized to `WF-002` and the Project aggregate,
    # exactly as `Workflows::Wf013::InvitationLedger` specializes it to WF-013.
    #
    # Only generic plumbing lives here. The workflow-specific decisions — the
    # `project.create` capability, the draft creation, the ProjectCreated type and
    # the creation first-match order — stay visible in the handler.
    #
    # Including handlers MUST define `ACTION`, `TARGET_TYPE` and
    ...
    module ProjectLedger
      ...
      # One canonically-encoded ProjectCreated envelope with actor attribution, the
      # Organization's real id and epoch, the Project id and its resulting state
      # version, and a null service identity (contracts/S-03.json event_payload).
      def write_event(store, ids, org, ctx, command, actor, now, aggregate_version, project_id,
                      request_sha256, key_digest, type, profile, extra = {})
        envelope = {
          "account_id" => actor.account_id, "actor_id" => actor.account_id,
          "affected_entity_id" => project_id, "affected_entity_type" => self.class::TARGET_TYPE,
          ...
          "outcome" => "success", "project_id" => project_id, "reason_code" => nil,
          "requester_account_id" => actor.account_id, "schema_version" => "1.0",
          "service_identity_id" => nil, "state_version" => aggregate_version, "workflow_id" => "WF-002"
        }.merge(extra)
        ...
        store.insert_event(
          ...
          aggregate_id: project_id, aggregate_version: aggregate_version, partition_month: month(now),
      ...
      def write_result_success(store, ids, command, ctx, org, actor, now, payload, project_id)
        store.insert_command_result(
          ...
          target_refs: JSON.generate({ self.class::TARGET_TYPE => project_id }),
      ...
      # An audited no-state outcome for an authenticated actor: execution,
      # authorization decision, audit and result, no Project row and no event. The
      # authorization decision reflects the real evaluation (an allow that was then
      # refused for tenant or input reasons is recorded as an allow), so the audit
      # trail distinguishes "no authority" from "authorized but refused".
      ...
      def schema_failure(command, ctx) = in_memory_failure(command, ctx, "project_schema_unsupported")

      # A pre-transaction failure with no authenticated actor and no proved context
      # to write under: the schema and profile validation reasons, and the
      # authentication reasons (invalid Session, inactive Account, inactive
      # Organization) — matching the WF-013 contract for the same class of failure.
