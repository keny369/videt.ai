...
module Workflows
  module Wf004
    # The ledger plumbing for the WF-004 Source commands: the durable command
    # execution, the authorization decision recorded on allow AND deny, the audit
    # record, the SourceRegistered event envelope, the command result and the
    # idempotency record. It is the exact WF-002 shape — the same
    # `IdentityAccess::Infrastructure::ActorLedgerWriters` platform ledgers and the
    # same canonical envelope — specialized to `WF-004` and the Source aggregate.
    #
    ...
    module SourceLedger
      ...
      # One canonically-encoded SourceRegistered envelope. The provenance actor and
      # command are the actor's Account and the command id; the payload carries the
      # canonical host/root and the registration/normalization versions, and no
      # submitted email, identity subject, credential or raw authorization token
      # (contracts/S-04.json event_payload).
      def write_event(store, ids, org, ctx, command, actor, now, aggregate_version, source_id,
                      request_sha256, key_digest, type, profile, extra = {})
        envelope = {
          "account_id" => actor.account_id, "actor_id" => actor.account_id,
          "affected_entity_id" => source_id, "affected_entity_type" => self.class::TARGET_TYPE,
          ...
          "service_identity_id" => nil, "source_id" => source_id, "state_version" => aggregate_version,
          "workflow_id" => "WF-004"
        }.merge(extra)
        ...
        store.insert_event(
          ...
          aggregate_id: source_id, aggregate_version: aggregate_version, partition_month: month(now),
      ...
      def write_result_success(store, ids, command, ctx, org, actor, now, payload, source_id)
        store.insert_command_result(
          ...
          target_refs: JSON.generate({ self.class::TARGET_TYPE => source_id }),
      ...
      # An audited no-Source outcome for an authenticated actor: execution,
      # authorization decision, audit and result, no Source row and no event.
      def deny(command:, ctx:, store:, auth_store:, actor:, decision:, org:, now:, request_sha256:,
        ...
        write_audit(store, ids[:audit], org, ctx, command, command.command_id, actor,
                    ...
                    payload: { "outcome" => "failure", "internal_reason" => internal,
                               ...
                               "project_id" => command.project_id }, now:)
      ...
      def schema_failure(command, ctx) = in_memory_failure(command, ctx, "source_request_schema_unsupported")

      # A pre-transaction failure with no authenticated actor and no proved context
      # to write under: the URI/host validation reasons and the authentication
