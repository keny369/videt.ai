...
module IdentityAccess
  module Infrastructure
    # Persistence for the WF-004 service-only Source Scope Change Request expiry
    # (S-06-005), written in the worker's unit of work under the action's Organization
    # context.
    #
    # Service-attributed throughout: `service_identity_id` is set and `actor_id` stays
    # null. A timed expiry is the source-scope lifecycle service's act, never a human's
    # (contracts/S-06.json MTX-029: `ExpireSourceScopeChange` is service-only, executed
    # by the lifecycle service at `due_at_utc`). This is the VerificationExpiryStore shape
    # specialized to `source_scope_change_requests` and stamped `WF-004`. It takes the same
    # per-Source advisory lock as ProposeSourceScopeChange/Decide/Cancel, so expiry
    # serializes against them for the Source.
    class SourceScopeChangeExpiryStore
      ...
      # The SAME lock key the decision/proposal paths use, so an expiry and a decision for
      # the Source cannot interleave (BACKGROUND/ADR-026 concurrency: expiry wins at due_at).
      ...
      def read(id)
        exec(<<~SQL, [id]).to_a.first
          SELECT id, organization_id, project_id, source_id, state, state_version, due_at_utc
          FROM source_scope_change_requests WHERE id = $1::uuid
        SQL
      end

      # Transition exactly one pending Request to expired, guarded on the expected state
      # version; the decision facts stay NULL (an expiry is not a decision) and only
      # terminal_at_utc is stamped. Changes no policy and no Source state. Returns row count.
      def expire(id, expected_version, now)
        exec(<<~SQL, [id, expected_version, iso(now)]).cmd_tuples
          UPDATE source_scope_change_requests
          SET state = 'expired', terminal_at_utc = $3::timestamptz,
