...
module IdentityAccess
  module Infrastructure
    # Persistence for the WF-003 service-only Verification Request expiry, written in
    ...
    # null. A timed expiry is the verification lifecycle service's act, never a human's
    # (contracts/S-05.json: automated slots and expiry are executed only by the named
    # lifecycle service). This is the RoleExpiryStore shape specialized to
    # `verification_requests` and stamped `WF-003`.
    class VerificationExpiryStore
      ...
      def read(id)
        sql = <<~SQL
          SELECT id, organization_id, project_id, source_id, request_status, state_version,
                 expires_at_utc, challenge_ciphertext_reference
          FROM verification_requests WHERE id = $1::uuid
        SQL
        exec(sql, [id]).to_a.first
      end

      # Transition exactly one pending Request to expired with reason challenge_expired,
      # and null the challenge material (the F-02 envelope is erased separately in the
      # same transaction). Guarded on the expected state version; returns the row count.
