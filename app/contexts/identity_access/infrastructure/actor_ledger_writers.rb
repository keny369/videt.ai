...
module IdentityAccess
  module Infrastructure
    # The actor-attributed platform ledger writers, shared by the WF-013
    # administrator commands.
    #
    # Every store in this context has carried its own copies of these five
    # inserts. That was fine while there was one such command; it is not a reason
    # to copy them a third time. The writers are identical by contract — the row
    # shapes are fixed by schemas/POSTGRESQL_SCHEMA.md :210, :211, :212 and the
    # audit/event registries — so they live here once and each store keeps only
    # its own domain reads and transitions.
    #
    # Actor-attributed: `actor_id` is set and `service_identity_id` stays null, so
    # `exactly_one_actor_or_service` holds and a human organization actor is never
    # recorded as a service. A service-executed command uses its own store; it
    # does not pass a service identity through here.
    module ActorLedgerWriters
