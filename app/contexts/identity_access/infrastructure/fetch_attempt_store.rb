...
module IdentityAccess
  module Infrastructure
    ...
    class FetchAttemptStore
      ...
      # Persist a fetch attempt's immutable identity/input before its ScheduledAction exists. The
      # execution lease is taken later, when the worker actually begins the attempt.
      def prepare(row)
        ...
        params = [row[:id], iso(row[:now]), row[:correlation_id], row[:causation_id],
                  ...
                  row[:crawl_policy_version], row[:reserved_bytes], iso(row[:deadline_at])]
      ...
      # Take the execution lease on a prepared attempt row just before the worker begins it. A
      # duplicate live claim is a no-op so the caller can release the ScheduledAction lease and let
      # the in-flight attempt finish or expire.
      def claim_prepared(id, expected_version, claim_owner, now)
        query(<<~SQL, [id, expected_version, claim_owner, LEASE_SECONDS, iso(now)]).cmd_tuples
          UPDATE fetch_attempts
          SET claim_owner = $3::uuid, claim_generation = claim_generation + 1,
              claimed_at = $5::timestamptz,
              lease_expires_at = $5::timestamptz + ($4 || ' seconds')::interval,
              checkpoint_version = checkpoint_version + 1, updated_at = $5::timestamptz
          WHERE id = $1::uuid AND checkpoint_version = $2
            AND claim_owner IS NULL AND outcome IS NULL
        SQL
      end
      ...
      def find_by_identity(organization_id:, crawl_host_gate_id:, frontier_entry_id:, kind:, attempt_number:)
        params = [organization_id, crawl_host_gate_id, frontier_entry_id, kind, attempt_number]
        query(<<~SQL, params).to_a.first
          SELECT * FROM fetch_attempts
          WHERE organization_id = $1::uuid
            AND crawl_host_gate_id = $2::uuid
            AND crawl_frontier_entry_id IS NOT DISTINCT FROM $3::uuid
            AND request_kind = $4
