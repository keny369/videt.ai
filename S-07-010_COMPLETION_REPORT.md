...
| Gate | Result |
| --- | --- |
| `bundle exec rspec` | `2981 examples, 0 failures` (2584 at this tranche; +29 from the registration-access transport work, then +61 from the WF-003 verification surface, the crawl-detail screen and the two outbound-transport defects, +73 from the WF-006 evaluation input gate and the S-08 parsing pipeline, +170 from the S-09/S-12 WF-007 check-catalogue slice and the S-08 external-measurement intake boundary, +15 from S-07-007's in-crawl link discovery and the autoloadable-constant guard, +8 from giving WF-001 the ratified first-Project body so the genesis Project carries a local-presence decision, +5 from FU-72 making both Project-creation forms able to assert a local presence, +2 from FU-59's Role Assignment INSERT guard, +1 from FU-56 bounding RaceHarness's connections, +2 from FU-42's spec-constant-scope check, +6 from FU-60 binding `AuthorityFixture` to the production write-authority derivation and selection, +5 from FU-61 deriving the protected-write set from the repository and proving every registered write is drivable, +3 from FU-55 binding `GovernedWriteSentinel`'s hook to the real `PG::Connection` door and refusing an empty census, +4 from FU-50 comparing the capability CTE across the three protected writes, and +13 from the owner-approval release service — the `measurement_set.activate` holder, its refusals, its raced version guard, and an approved-but-expired package refused by CHK-AIP-001 as stale rather than as missing, all of which followed it) |
| `bundle exec rspec spec/architecture` | 267 examples, 0 failures |
