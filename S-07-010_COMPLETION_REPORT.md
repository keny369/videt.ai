...
| Gate | Result |
| --- | --- |
| `bundle exec rspec` | `3055 examples, 0 failures` (2584 at this tranche; +29 from the registration-access transport work, then +61 from the WF-003 verification surface, the crawl-detail screen and the two outbound-transport defects, +73 from the WF-006 evaluation input gate and the S-08 parsing pipeline, +170 from the S-09/S-12 WF-007 check-catalogue slice and the S-08 external-measurement intake boundary, +15 from S-07-007's in-crawl link discovery and the autoloadable-constant guard, +8 from giving WF-001 the ratified first-Project body so the genesis Project carries a local-presence decision, +5 from FU-72 making both Project-creation forms able to assert a local presence, +2 from FU-59's Role Assignment INSERT guard, +1 from FU-56 bounding RaceHarness's connections, +2 from FU-42's spec-constant-scope check, +6 from FU-60 binding `AuthorityFixture` to the production write-authority derivation and selection, +5 from FU-61 deriving the protected-write set from the repository and proving every registered write is drivable, +3 from FU-55 binding `GovernedWriteSentinel`'s hook to the real `PG::Connection` door and refusing an empty census, +4 from FU-50 comparing the capability CTE across the three protected writes, +13 from the owner-approval release service — the `measurement_set.activate` holder, its refusals, its raced version guard, and an approved-but-expired package refused by CHK-AIP-001 as stale rather than as missing — and +16 from ADR-145's three follow-ups: 5 giving `failed_attempts` a shape it can refuse a malformed row against, 2 driving the review-failure transition that now writes it, 2 deriving the recorded failures from the review records, 2 resolving each protected write's organization operand back through its own parameter list, and 5 requiring the two row-bound writes to refuse an organization that is not the authenticated one, and +6 from ADR-146's WF-005 event-profile conformance gate, which derives the closed profiles from API_CONTRACTS.md and compares them to the emitted `event_registry.event_bytes`, and +10 from ADR-147's three protected-write residues: 6 requiring the capability CTE to carry `confers?`'s protected limb and `cancel` to write only inside its own tenant, and 4 running PROOF 264's lock-order measurement at every write in the registry instead of one, and +5 from ADR-148's automation-record chain: the plan parsing as YAML, `current_tranche` and `completed_blocks` naming blocks the plan defines, an in-flight report that exists, and Preflight's working-state limb run against the real record, and +22 from ADR-149's instrument and schema-convention repairs: 7 giving `ProtectedEffectDoor` a verdict for a statement PostgreSQL will not plan and proving it catches a TRUNCATE and a COPY that really wrote, 1 holding BUILD_PLAN's block statuses to `completed_blocks`, 2 leaving the Permission Baseline's role and mode limbs with no production caller outside the module, 2 refusing a read_only OrganizationAdmin's reactivation and admitting a standard one, 3 keeping the valid narrowing policies when one stored policy is unreadable, 2 binding every Service Identity attribution column, 2 requiring :128's three-column key of every Project-owned crawl table, 2 refusing an unregistered limit-decision author, and 1 requiring an exhausted run to stop claiming host-gate slots, and +15 from ADR-150: 7 for FU-2/FU-49 — 5 requiring the capability CTE to refuse a grant whose scope does not contain an Organization-scope target and to admit both spellings of Organization scope, 1 refusing the same actor through the production `ActivateCrawlPolicy` handler, and 1 binding `AuthorityFixture` to the new containment operand — and 8 for FU-76: 6 deriving the ratified role/mode/persona constraint from both the constant and `pg_get_constraintdef` and holding the tuple columns immutable, and 2 sweeping every tuple the two domain CHECKs admit against `role_assignments` and `invitations`) |
| `bundle exec rspec spec/architecture` | 286 examples, 0 failures |
