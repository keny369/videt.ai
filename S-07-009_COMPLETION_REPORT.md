...
| **D9** the mutation harness | the round-two `broken` correction was applied to `replay_trigger` alone, leaving the file path — all 82 of the 92 definitions that are file mutations — recording the strongest kills as proving nothing | one classifier, `MutationHarness.classify`, called by both paths and proved by its own spec (R15-CONC-2 found the D7 closure itself reached only one path) |
